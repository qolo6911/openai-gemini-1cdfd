  ...
  for (const params of combinations) {
    try {
      ...
      if (verbose && completed % 10 === 0) {
        console.log(`进度: ${completed}/${combinations.length} (${(completed/combinations.length*100).toFixed(1)}%)`);
      }
    } catch (error) {
      console.error(`参数组合测试失败:`, params, error.message);
    }
  }
...
async function main() {
  ...
  console.log('XAUUSD布林带策略参数优化');
  console.log('Parameter Optimization for XAUUSD Bollinger Strategy');
  console.log('='.repeat(100));
  console.log('');
